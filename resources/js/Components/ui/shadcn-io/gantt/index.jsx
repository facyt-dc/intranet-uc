...
export const GanttFeatureItemCard = ({
  id,
  children,
  style //Se agrega style para poder asignarle el color
}) => {
  const [, setDragging] = useGanttDragging();
  ...
  return (
    <Card className="h-full w-full rounded-md bg-background p-2 text-xs shadow-sm" style={style}>
      <div
        className={cn(
    ...
    <div
      ...
      <div
        ...
        <DndContext
          ...
          sensors={[mouseSensor]}>
          <GanttFeatureItemCard id={feature.id} style={style}>
            {children ?? (
...
export const GanttProvider = ({
  ...
  const [startYear, endYear] = useMemo(() => {
    ...
    for (const feature of features) {
      ...
      const end = getYear(feature.endAt);
  ...
  const createDynamicInitialTimelineData = (start, end) => {
    const data = [];
  ...
  const [timelineData, setTimelineData] = useState(
    createDynamicInitialTimelineData(startYear, endYear)
  );
  ...
  return (
    <GanttContext.Provider
      ...
      <div
        className={cn(
          'gantt relative grid w-full max-h-[75vh] flex-none select-none overflow-auto rounded-sm bg-secondary',
          range,
          className
        )}
        ref={scrollRef}
